<template>
  <div>
    ...
    <div>
      <el-button @click="toggleAmbient"
        >环境光 ({{ ambientFlag ? "开" : "关" }})</el-button
      >
      <el-button @click="togglePoint"
        >点光源 ({{ pointFlag ? "开" : "关" }})</el-button
      >
      <el-button @click="toggleDirectional"
        >平行光 ({{ directionalFlag ? "开" : "关" }})</el-button
      >
      <el-button @click="toggleSpot"
        >聚光源 ({{ spotFlag ? "开" : "关" }})</el-button
      >
    </div>
  </div>
</template>
...
// !!!!!要有 this.renderer.shadowMap.enabled = true
export default {
  name: "",
  components: {},
  data() {
  ...
  methods: {
    ...
    toggleDirectional() {
      if (this.directionalFlag) {
        ...
        this.directionalHelper = new THREE.DirectionalLightHelper(
    ...
    toggleSpot() {
      if (this.spotFlag) {
        ...
        this.spotLight.position.set(-1500, 300, -800);
